<template>
  ...
  >
    ...
    <div class="flex flex-1 gap-1">
      ...
      <span class="text-text-secondary !f-text-14-14 pt-2 text-start ps-xs"
        >مدل : <span class="text-primary-alternate">{{ desc }}</span>
      </span>
      <div class="flex justify-between w-full">
        <span class="text-text-secondary !f-text-14-14 pt-2 text-start ps-xs"
          >منطقه : <span class="text-primary-alternate">{{ region }}</span>
        </span>
        <StarComponent
          starClasses="!w-5 !h-5"
    ...
    <div class="flex !overflow-visible basis-full">
      ...
      >
        ...
        <div class="w-full !flex justify-between gap-2 py-2 px-3 items-center">
          <span class="f-text-11-14 text-text-secondary line-through">
            {{ price }} تومان
          </span>
          <span class="f-text-18-22 text-primary"> {{ finalPrice }} تومان</span>
        </div>
      </QBtn>
    </div>
  </div>
